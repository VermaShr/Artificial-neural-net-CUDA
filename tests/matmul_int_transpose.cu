...
int main(int argc, char** argv)
{
    ...
    h_x = (int*)malloc(width*sizeof(int));
    h_A = (int*)malloc(height*width*sizeof(int));
    h_B = (int*)malloc(height*sizeof(int));
    cudaMalloc((void**)&d_x, width*sizeof(int));
    cudaMalloc((void**)&d_A, height*width*sizeof(int));
    cudaMalloc((void**)&d_B, height*sizeof(int));
    ...
    for (int i = 0; i < height; i++)
    {
        ...
        {
            h_A[i*width + j] = rand() * (10) - 5;
        }
    }

    // Copy vectors from host memory to device memory
    cudaMemcpy(d_x, h_x, width*sizeof(int), cudaMemcpyHostToDevice);
    ...
    cudaMemcpy(h_B, d_B, height*sizeof(int), cudaMemcpyDeviceToHost);
    ...
    for (int i = 0; i < height; i++)
    {
        ...
        if (h_B[i] != result[i])
        {
            printf("ERROR: expected h_B[%i] = %i but received %i\n", i, result[i], h_B[i]);
            correct = false;
            break;
        }
    }
